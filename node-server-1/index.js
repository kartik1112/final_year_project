...
app.post("/resend-verification", async (req, resp) => {
  try {
    ...
    resp
      .status(500)
      .send({ message: "Failed to resend the verification email." });
  }
});
...
app.post(
  "/event",
  event_profile_up.single("event_photo"),
  async (req, resp) => {
    if (req.body.event_name && req.body.created_id) {
      try {
        ...
        if (userCheck) {
          ...
          resp
            .status(404)
            .send({ result: "User account is not valid or not available" });
        }
      } catch (error) {
        resp
          .status(500)
          .send({ result: "An error occurred", error: error.message });
      }
    } else {
      resp
        .status(400)
        .send({ result: "Event name and created_id are required" });
    }
  }
);
...
app.post("/display_event", async (req, resp) => {
  try {
    ...
    resp
      .status(500)
      .send({ message: "An error occurred while retrieving events" });
  }
});
//---------------------------------------------------------------------------------------------------------
app.post("/in-event", async (req, resp) => {
  ...
  try {
    ...
    resp
      .status(500)
      .send({
        result: "An error occurred while retrieving images",
        error: error.message,
      });
  }
});
...
app.post("/photo", upload.array("name", 10), async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({
        result: "An error occurred while uploading images",
        error: error.message,
      });
  }
});
...
app.delete("/delete-event", async (req, res) => {
  try {
    ...
    if (!_id) {
      return res
        .status(400)
        .send({ message: "Event is Missing! Please Reload" });
    }
    ...
    if (!event) {
      return res
        .status(404)
        .send({ message: "Event not found! Reload the page." });
    }

    if (event.event_photo) {
      const coverImage_path = path.join(
        __dirname,
        "event_profile",
        event.event_photo
      );
...
app.delete("/delete-image", async (req, res) => {
  try {
    ...
    if (!name || !_id) {
      return res
        .status(400)
        .json({ success: false, message: "Missing images detail's" });
    }

    const result = await Photo.findOneAndDelete({
      name,
      ...
      if (event) {
        if (event.pin == pin) {
          ...
          resp
            ...
            .send({
              result:
                "Pin is wrong! Contact the photographer to provide the correct (Pin)",
            });
        }
      } else {
        resp
          .status(404)
          .send({ result: "Event not found. Please check the Event ID." });
      }
    } else {
...
app.post("/studio", async (req, resp) => {
  const { studio_name, phone_no, address, offer, description, create_by } =
    req.body;
  ...
  if (create_by && studio_name && phone_no) {
    try {
      ...
      if (existingStudio) {
        ...
        if (updatedStudio) {
          return resp
            .status(200)
            .send({ message: "Updated your details!", updatedStudio });
        } else {
          return resp
            .status(404)
            .send({ message: "Failed to update your details!" });
        }
      } else {
        // Create a new record
        ...
        if (result) {
          return resp
            .status(200)
            .send({ message: "Saved your details!", studio: result });
        } else {
          return resp
            .status(404)
            .send({ message: "Failed to save your details!" });
        }
      }
    } catch (error) {
    ...
    return resp
      .status(400)
      .send({ message: "Studio name, Phone No, and Created By are required" });
  }
});
...
app.get("/exist-studio", async (req, res) => {
  try {
    ...
    if (create_by) {
      ...
      return res
        .status(400)
        .send({ message: "create_by parameter is required" });
    }
  } catch (error) {
    console.error(error);
...
app.post("/newPassword-verify-otp", async (req, res) => {
  ...
  if (otpStorage[email] === otp) {
    ...
    if (update) {
      res
        .status(200)
        .json({ message: "Your Password is successfully Change!" });
    } else {
      res
        .status(404)
        .send({ message: "User is not available or Server error" });
    }
  } else {
    res
      .status(400)
      .json({
        message: `Invalid OTP or Expire! Regenrate OTP against ${email}`,
      });
  }
});
...
app.put("/events/:id", async (req, res) => {
  ...
  try {
    ...
    if (!event_name && !pin) {
      return res
        .status(400)
        .json({ message: "Not Provide event_name or pin to update." });
    }
